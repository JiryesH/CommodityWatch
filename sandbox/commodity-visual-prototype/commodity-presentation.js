...
function deriveGroup(seriesKey) {
...
function buildVisual(seriesKey, row) {
  const group = deriveGroup(seriesKey);
...
function mergeSeriesOption(row, latestRow) {
  ...
  return {
    ...
    group: deriveGroup(row.series_key),
...
export function buildCommodityDefinitions(seriesRows, latestRows) {
  ...
  const optionsBySeriesKey = new Map(seriesOptions.map((option) => [option.seriesKey, option]));
  const consumedSeriesKeys = new Set();
  const definitions = [];

  GROUPED_CARD_PRESETS.forEach((groupedPreset) => {
    const groupedOptions = groupedPreset.seriesKeys
      .map((seriesKey) => optionsBySeriesKey.get(seriesKey))
      .filter(Boolean);
