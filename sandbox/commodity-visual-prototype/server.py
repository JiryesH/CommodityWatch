...
def resolve_database_url(raw_database_url: str, backend_root: Path) -> str:
    ...
    if url.drivername == "sqlite" and url.database and url.database != ":memory:" and not os.path.isabs(url.database):
...
def build_config(app_root: Path = APP_ROOT) -> AppConfig:
    ...
    return AppConfig(app_root=app_root, backend_root=backend_root, database_url=database_url, port=port)
...
def make_handler(repository: CommodityRepository, app_root: Path):
    class CommodityRequestHandler(SimpleHTTPRequestHandler):
        ...
        def handle_api_get(self, parsed) -> None:
            ...
            suffix = parsed.path.removeprefix("/api/commodities/")
    ...
    return CommodityRequestHandler


def create_server(config: AppConfig) -> ThreadingHTTPServer:
    repository = CommodityRepository(create_db_engine(config.database_url))
    handler = make_handler(repository, config.app_root)
...
def serve(config: AppConfig) -> None:
    ...
    print(f"Serving {config.app_root} on http://127.0.0.1:{server.server_port}")
    print(f"Reading published commodity data from {database_url}")
