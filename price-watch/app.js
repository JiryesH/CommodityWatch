...
class CommodityWatchEngine {
  constructor(apiClient) {
    ...
    this.ui = {
      ...
      catalogEmptyTitle: document.getElementById("catalog-empty-title"),
      catalogEmptyCopy: document.getElementById("catalog-empty-copy"),
  ...
  bindGlobalEvents() {
    ...
    if (this.ui.searchInput) {
      this.ui.searchToggle?.addEventListener("click", () => {
        if (this.ui.navSearch?.classList.contains("open")) {
          this.closeSearch();
          return;
        }
    ...
    this.ui.toTopButton?.addEventListener("click", () => {
    ...
    document.addEventListener("click", (event) => {
      if (this.ui.navSearch?.classList.contains("open") && !this.ui.navSearch.contains(event.target)) {
        this.closeSearch();
      }
    });
  }
  ...
  setShellInteractivity(isDetailOpen) {
    [this.ui.nav, this.ui.filterWrap, this.ui.statusBar, this.ui.pageShell, this.ui.footer].forEach((element) => {
      if (!element) {
        return;
      }
  ...
  openSearch() {
    if (!this.ui.navSearch || !this.ui.searchToggle || !this.ui.searchInput || this.ui.searchInput.disabled) {
      return;
    }

    this.ui.navSearch.classList.add("open");
    this.ui.searchToggle.setAttribute("aria-expanded", "true");
  }

  closeSearch() {
    if (!this.ui.navSearch || !this.ui.searchToggle) {
      return;
    }

    this.ui.navSearch.classList.remove("open");
    this.ui.searchToggle.setAttribute("aria-expanded", "false");
  }

  syncToTopButton() {
    if (!this.ui.toTopButton) {
      return;
    }

    const shouldShow = !document.body.classList.contains("detail-open") && window.scrollY > TO_TOP_SCROLL_THRESHOLD;
  ...
  updateCatalogState(visiblePriceCount) {
    ...
    this.ui.catalogEmpty.hidden = false;
    this.ui.catalogEmptyTitle.textContent = "No prices match your current filters.";
    this.ui.catalogEmptyCopy.textContent = "Try a broader commodity name, benchmark, or category.";
  }

  setSearchAvailability(enabled) {
    if (!this.ui.searchInput) {
      return;
    }

    this.ui.searchInput.disabled = !enabled;
  ...
  getHeadlineCacheKey(seriesOption) {
    return `${seriesOption.seriesKey}:${RELATED_HEADLINES_LIMIT}`;
  }
  ...
  async ensureRelatedHeadlines(definition, seriesOption) {
    const cacheKey = this.getHeadlineCacheKey(seriesOption);
    const existing = this.headlineCache.get(cacheKey);

    if (existing && (existing.status === "loading" || existing.status === "ready" || existing.status === "error")) {
      return;
    }

    this.headlineCache.set(cacheKey, {
      status: "loading",
      rows: [],
      error: null,
    });
    ...
    try {
      const rows = await this.apiClient.getRelatedHeadlines(seriesOption.seriesKey, {
        limit: RELATED_HEADLINES_LIMIT,
      });
      this.headlineCache.set(cacheKey, {
        status: "ready",
        rows,
  ...
  getDetailViewModel(definition) {
    ...
    const headlineCacheKey = this.getHeadlineCacheKey(activeSeries);
    const headlineEntry = this.headlineCache.get(headlineCacheKey) || {
      status: "loading",
      rows: [],
      error: null,
    };

    if (!this.headlineCache.has(headlineCacheKey)) {
      this.ensureRelatedHeadlines(definition, activeSeries);
    }
    ...
    return {
      ...
      headlinesStatus: headlineEntry.status,
      error: historyEntry.error,
      headlinesError: headlineEntry.error,
      rows: historyEntry.rows,
      headlines: headlineEntry.rows,
      series,
  ...
  bindDetailControls(definition) {
    ...
    const downloadButton = this.ui.detailBack.querySelector("[data-detail-download]");
    ...
    downloadButton?.addEventListener("click", () => {
      this.downloadDetailCsv(definition);
    });
  }

  downloadDetailCsv(definition) {
    const detailView = this.getDetailViewModel(definition);
    const activeSeries = detailView.activeSeries;

    if (!activeSeries || detailView.historyStatus !== "ready" || !detailView.rows.length) {
      return;
    }

    const filename = buildCsvFilename(activeSeries.seriesKey, detailView.timeframe.id);
    const csvContent = buildHistoryCsv(detailView.rows);
    triggerTextDownload(filename, csvContent, "text/csv;charset=utf-8;");
  }
  ...
  renderDetailBack(definition, detailView) {
    ...
    const sourceMarkup = activeSeries.sourceUrl
      ? `<a class="detail-link" href="${escapeHtml(activeSeries.sourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(
    ...
    `;
    const relatedHeadlinesSection = buildRelatedHeadlinesSection(
      detailView.headlines,
      detailView.headlinesStatus,
      detailView.headlinesError
    );

    const canDownloadCsv = detailView.historyStatus === "ready" && detailView.rows.length > 0;
    const sourceRow = `
      <div class="detail-source-row">
        ...
        <button
          class="detail-export-button"
          type="button"
          data-detail-download
          aria-label="Download visible price history as CSV"
          ${canDownloadCsv ? "" : "disabled"}
        >
          CSV
        </button>
      </div>
...
function buildRelatedHeadlinesSection(headlines, status, errorMessage) {
  let bodyMarkup = '<p class="detail-related-state">No related headlines available.</p>';

  if (status === "loading") {
    bodyMarkup = '<p class="detail-related-state">Loading related headlines...</p>';
  } else if (status === "error") {
    bodyMarkup = `
      <p class="detail-related-state is-error">Related headlines unavailable.</p>
      ${errorMessage ? `<p class="detail-related-error-meta">${escapeHtml(compactErrorMessage(errorMessage))}</p>` : ""}
    `;
    ...
    bodyMarkup = `
      <ul class="detail-related-list">
        ${headlines
          .map((headline) => {
            const titleMarkup = headline.link
              ? `<a class="detail-related-link" href="${escapeHtml(headline.link)}" target="_blank" rel="noopener noreferrer">${escapeHtml(
                  headline.title
                )}</a>`
              : `<span class="detail-related-link static">${escapeHtml(headline.title)}</span>`;

            return `
              <li class="detail-related-item">
                <p class="detail-related-title">${titleMarkup}</p>
                <p class="detail-related-meta">
                  <span>${escapeHtml(headline.source || "Unknown source")}</span>
                  <span>${escapeHtml(formatTimestamp(headline.published))}</span>
                </p>
              </li>
            `;
          })
  ...
  return `
    <section class="detail-related" aria-labelledby="detail-related-headlines-title">
      <div class="detail-related-head">
        <p class="detail-related-kicker">HeadlineWatch</p>
        <h4 class="detail-related-heading" id="detail-related-headlines-title">Related headlines</h4>
      </div>
      ${bodyMarkup}
    </section>
  `;
