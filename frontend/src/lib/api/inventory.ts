...
function mapIndicatorLatestResponse(payload: RawIndicatorLatestResponse): IndicatorLatestResponse {
  return {
    ...
    latest: {
      ...
      commodityWatchUpdatedAt: payload.latest.commoditywatch_updated_at,
...
function mapIndicatorDataResponse(payload: RawIndicatorDataResponse): IndicatorDataResponse {
  ...
  return {
    indicator: {
      ...
      periodType: payload.indicator.period_type ?? null,
      marketingYearStartMonth: payload.indicator.marketing_year_start_month ?? null,
      isSeasonal: payload.indicator.is_seasonal,
    },
    ...
    metadata: {
      ...
      latestPeriodEndAt: payload.metadata.latest_period_end_at,
      latestVintageAt: payload.metadata.latest_vintage_at,
  ...
  return {
    ...
    cards: payload.cards.map((card) => {
      ...
      return {
        ...
        periodType: card.period_type ?? null,
        marketingYearStartMonth: card.marketing_year_start_month ?? null,
        isSeasonal: card.is_seasonal,
        ...
        seasonalMedian: convertUnitValue(card.seasonal_median ?? null, card.unit),
        seasonalP10: convertUnitValue(card.seasonal_p10 ?? null, card.unit),
        seasonalP25: convertUnitValue(card.seasonal_p25 ?? null, card.unit),
        seasonalP75: convertUnitValue(card.seasonal_p75 ?? null, card.unit),
        seasonalP90: convertUnitValue(card.seasonal_p90 ?? null, card.unit),
        signal: card.signal,
        sparkline: card.sparkline.map((value) => convertUnitValue(value, card.unit) ?? value),
        periodEndAt: card.latest_period_end_at,
        releaseDate: card.latest_release_date,
        commodityWatchUpdatedAt: card.commoditywatch_updated_at,
        freshness: freshnessFor(card.frequency ?? undefined, card.latest_release_date, card.commoditywatch_updated_at),
        stale: card.stale,
